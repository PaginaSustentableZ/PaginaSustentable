...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT cantidad_playera_b, precio_playera_b FROM inventario3 WHERE talla_playera_b = '$talla'";

    // Ejecutar la consulta y obtener el resultado
    $resultado = mysqli_query($conexion, $sql);
    $fila = mysqli_fetch_assoc($resultado);
    $cantidad_actual = $fila['cantidad_playera_b'];
    // Obtener el usuario activo
    $usuario = $_SESSION['usuario'];
    $correoD = $_SESSION['correo'];

    $precio_playera = $fila['precio_playera_b'];
    ...
    $sql = "INSERT INTO playera (cantidad, talla, precio, dinero_final, nombre) VALUES ('$cantidad', '$talla', '$precio_total', '$dinero_final', (SELECT nombre_playera_b FROM inventario3 WHERE talla_playera_b = '$talla'))";
    mysqli_query($conexion, $sql);

    // Actualizar la cantidad en inventario
    $sql = "UPDATE inventario3 SET cantidad_playera_b = $cantidad_restante WHERE talla_playera_b = '$talla'";
    mysqli_query($conexion, $sql);
...
$sql = 'SELECT DISTINCT talla_playera_b FROM inventario3';

// Ejecutar la consulta y obtener el resultado
$resultado = mysqli_query($conexion, $sql);
$tallas = array();
while ($fila = mysqli_fetch_assoc($resultado)) {
    $tallas[] = $fila['talla_playera_b'];
}
