...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT cantidad_playera_c, precio_playera_c FROM inventario4 WHERE talla_playera_c = '$talla'";

    // Ejecutar la consulta y obtener el resultado
    $resultado = mysqli_query($conexion, $sql);
    $fila = mysqli_fetch_assoc($resultado);
    $cantidad_actual = $fila['cantidad_playera_c'];
    // Obtener el usuario activo
    $usuario = $_SESSION['usuario'];
    $correoD = $_SESSION['correo'];

    $precio_playera = $fila['precio_playera_c'];
    ...
    $sql = "INSERT INTO playera (cantidad, talla, precio, dinero_final, nombre) VALUES ('$cantidad', '$talla', '$precio_total', '$dinero_final', (SELECT nombre_playera_c FROM inventario4 WHERE talla_playera_c = '$talla'))";
    mysqli_query($conexion, $sql);

    // Actualizar la cantidad en inventario
    $sql = "UPDATE inventario4 SET cantidad_playera_c = $cantidad_restante WHERE talla_playera_c = '$talla'";
    mysqli_query($conexion, $sql);
...
$sql = 'SELECT DISTINCT talla_playera_c FROM inventario4';

// Ejecutar la consulta y obtener el resultado
$resultado = mysqli_query($conexion, $sql);
$tallas = array();
while ($fila = mysqli_fetch_assoc($resultado)) {
    $tallas[] = $fila['talla_playera_c'];
}
