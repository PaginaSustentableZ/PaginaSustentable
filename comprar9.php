...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT cantidad_playera_h, precio_playera_h FROM inventario9 WHERE talla_playera_h = '$talla'";

    // Ejecutar la consulta y obtener el resultado
    $resultado = mysqli_query($conexion, $sql);
    $fila = mysqli_fetch_assoc($resultado);
    $cantidad_actual = $fila['cantidad_playera_h'];
    // Obtener el usuario activo
    $usuario = $_SESSION['usuario'];
    $correoD = $_SESSION['correo'];

    $precio_playera = $fila['precio_playera_h'];
    ...
    $sql = "INSERT INTO playera (cantidad, talla, precio, dinero_final, nombre) VALUES ('$cantidad', '$talla', '$precio_total', '$dinero_final', (SELECT nombre_playera_h FROM inventario9 WHERE talla_playera_h = '$talla'))";
    mysqli_query($conexion, $sql);

    // Actualizar la cantidad en inventario
    $sql = "UPDATE inventario9 SET cantidad_playera_h = $cantidad_restante WHERE talla_playera_h = '$talla'";
    mysqli_query($conexion, $sql);
...
$sql = 'SELECT DISTINCT talla_playera_h FROM inventario9';

// Ejecutar la consulta y obtener el resultado
$resultado = mysqli_query($conexion, $sql);
$tallas = array();
while ($fila = mysqli_fetch_assoc($resultado)) {
    $tallas[] = $fila['talla_playera_h'];
}
