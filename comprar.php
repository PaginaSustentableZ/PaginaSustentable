...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT cantidad_playera, precio_playera FROM inventario WHERE talla_playera = '$talla'";

    // Ejecutar la consulta y obtener el resultado
    $resultado = mysqli_query($conexion, $sql);
    $fila = mysqli_fetch_assoc($resultado);
    $cantidad_actual = $fila['cantidad_playera'];

    // Obtener el usuario activo
    $usuarioD = $_SESSION['usuario'];
    $correoD = $_SESSION['correo'];

    $precio_playera = $fila['precio_playera'];
    ...
    $sql = "INSERT INTO playera (cantidad, talla, precio, dinero_final, nombre) VALUES ('$cantidad', '$talla', '$precio_total', '$dinero_final', (SELECT nombre_playera FROM inventario WHERE talla_playera = '$talla'))";
    mysqli_query($conexion, $sql);

    // Actualizar la cantidad en inventario
    $sql = "UPDATE inventario SET cantidad_playera = $cantidad_restante WHERE talla_playera = '$talla'";
    mysqli_query($conexion, $sql);

    // Actualizar el atributo "dinero" en la tabla "usuarios"
    $sql = "UPDATE usuarios SET dinero = $dinero_final WHERE correo = '$correoD'";
    mysqli_query($conexion, $sql);

    // Mostrar mensaje de éxito
    echo '<p>Compra realizada con Éxito, La Sudadera llegará dentro de 5 días a tu Dirección</p>';
}

// Consulta SQL para obtener la talla de la playera
$sql = 'SELECT DISTINCT talla_playera FROM inventario';

// Ejecutar la consulta y obtener el resultado
$resultado = mysqli_query($conexion, $sql);
$tallas = array();
while ($fila = mysqli_fetch_assoc($resultado)) {
    $tallas[] = $fila['talla_playera'];
}
